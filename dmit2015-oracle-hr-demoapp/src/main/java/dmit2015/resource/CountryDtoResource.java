 ...
 */
@ApplicationScoped
@Path("countries")                // All methods in this class are associated this URL path
...
public class CountryDtoResource {
    ...
    public Response findCountryByIdCountryById(@PathParam("id") String countryId) {
        Country existingCountry = countryRepository.findById(countryId).orElseThrow(NotFoundException::new);
    ...
    public Response createCountryCountry(CountryDto dto, @Context UriInfo uriInfo) {
        ...
        String errorMessage = JavaBeanValidator.validateBean(newCountry);
    ...
    public Response updateCountryCountry(@PathParam("id") String id, CountryDto dto) {
        ...
        Country existingCountry = countryRepository
                .findById(id)
                .orElseThrow(NotFoundException::new);

        Country updatedCountry = CountryMapper.INSTANCE.toEntity(dto);

        String errorMessage = JavaBeanValidator.validateBean(updatedCountry);
        if (errorMessage != null) {
            return Response
                    .status(Response.Status.BAD_REQUEST)
                    .entity(errorMessage)
        ...
        CountryDto updatedDto = CountryMapper.INSTANCE.toDto(existingCountry);
    ...
    public Response deleteCountryCountry(@PathParam("id") String countryId) {
        ...
        try {
            countryRepository.delete(existingCountry);    // Removes the Country from being persisted
        } catch (Exception ex) {
            // Return a HTTP status of "500 Internal Server Error" containing the exception message
            return Response
                    .serverError()
